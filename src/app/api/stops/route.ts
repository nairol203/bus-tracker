...
export async function GET() {
  ...
  try {
    const fetchPromises = characters.map(async (char) => {
      const res = await fetch(`https://kvg-internetservice-proxy.p.networkteam.com/internetservice/services/lookup/stopsByCharacter?character=${char}`);
      if (!res.ok) return [];
    ...
    const uniqueStops = Array.from(new Map(allStops.map(stop => [stop.number || stop.id, stop])).values());
