	...
	return (
		<>
			{filterIdenticalAlerts(data.generalAlerts).map((alert, index) => (
				<div className='flex gap-2 rounded bg-yellow-400 p-2 text-black shadow dark:bg-yellow-600' key={`alert-${index}-${data.stopShortName}`}>
			...
			{data.routes
				...
				.map((route) =>
					filterIdenticalAlerts(route.alerts)
						...
						.map((alert, index) => (
							<div
								className='grid gap-1 rounded bg-yellow-400 p-2 text-black shadow dark:bg-yellow-600 md:flex md:justify-between'
