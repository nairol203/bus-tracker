...
export default function KVGTable({
	...
	const filteredActual = data.actual.filter((stop) => (direction ? stop.direction === direction : true && routeId ? stop.routeId === routeId : true));
	...
	return (
		<div className='grid gap-1'>
			...
			{filteredActual.length ? (
				filteredActual
					.filter((stop) => (direction ? stop.direction === direction : true && routeId ? stop.routeId === routeId : true))
					.map((actual, index) => (
						<Link
