...
export default function DeparturesList({ stopId, stopName, onSelectTrip }: DeparturesListProps) {
  ...
  const { data, error, isLoading } = useSWR(`/api/departures?stopId=${stopId}`, fetcher, {
  ...
  if (error) {
    return (
      <div className="mt-8 flex flex-col items-center justify-center p-12 bg-surface/50 border border-red-500/50 rounded-2xl">
        ...
        <p className="text-red-400">Fehler beim Laden der Abfahrten. Bitte versuchen Sie es erneut.</p>
      </div>
    );
  }
  ...
  if (departures.length === 0) {
    return (
      <div className="mt-8 flex flex-col items-center justify-center p-12 bg-surface/50 border border-border rounded-2xl">
        ...
        <p className="text-muted">Keine anstehenden Abfahrten für {stopName}.</p>
      </div>
    );
  }

  const uniqueLines = Array.from(new Set(departures.map(d => d.patternText))).sort();
  const uniqueDirections = Array.from(new Set(departures.map(d => d.direction))).sort();
  ...
  const toggleLine = (line: string) => {
    const newLines = selectedLines.includes(line) ? selectedLines.filter(l => l !== line) : [...selectedLines, line];
    setSelectedLines(newLines);
    updateParams(newLines, selectedDirections);
  };

  const toggleDirection = (dir: string) => {
    const newDirs = selectedDirections.includes(dir) ? selectedDirections.filter(d => d !== dir) : [...selectedDirections, dir];
  ...
  const filteredDepartures = departures.filter(dep => {
    const lineMatch = selectedLines.length === 0 || selectedLines.includes(dep.patternText);
    const dirMatch = selectedDirections.length === 0 || selectedDirections.includes(dep.direction);
  ...
  return (
    ...
    >
      <div className="bg-brand/10 border-b border-border px-6 py-4 flex items-center justify-between">
        <h2 className="text-xl font-bold text-foreground">Abfahrten von {stopName}</h2>
        ...
        <AnimatePresence>
          {filteredDepartures.length === 0 ? (
            ...
            filteredDepartures.map((dep, idx) => {
              const isLeavingSoon = dep.actualRelativeTime > 0 && dep.actualRelativeTime < 300; // Less than 5 mins
          ...
          return (
            <motion.div 
              ...
              onClick={() => onSelectTrip(dep.tripId, dep.patternText, dep.direction)}
            ...
            >
              <div className="flex items-center space-x-4 min-w-0 flex-1">
                <div className={`w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-xl font-bold text-lg ${isLeavingSoon ? 'bg-brand text-white shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-background text-foreground'}`}>
                  {dep.patternText}
                </div>
                <div className="min-w-0 flex-1 pr-2">
                  <h3 className="font-semibold text-lg text-foreground group-hover:text-brand transition-colors truncate">{dep.direction}</h3>
                  <div className="text-sm flex items-center mt-1">
                    <Clock className="w-3 h-3 flex-shrink-0 mr-1 text-muted" />
                    {dep.plannedTime && dep.actualTime && showDiff ? (
                      <div className="flex items-center min-w-0 overflow-hidden">
                        <span className="line-through text-muted mr-2 flex-shrink-0">{dep.plannedTime}</span>
                        <span className={`font-medium flex items-center min-w-0 ${isDelayed ? 'text-red-400' : 'text-emerald-400'}`}>
                          <span className="flex-shrink-0">{dep.actualTime}</span>
                          <span className={`ml-2 text-[10px] px-1.5 py-0.5 rounded border uppercase tracking-wider flex-shrink-0 ${isDelayed ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'}`}>
                            {isDelayed ? 'Verspätet' : 'Früher'}
                          </span>
                        </span>
                      </div>
                    ) : (
                      <span className="text-muted flex-shrink-0">{dep.actualTime}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="text-right flex-shrink-0 ml-2">
                {dep.actualRelativeTime <= 0 ? (
                  <span className="text-brand font-bold animate-pulse">Jetzt</span>
                ) : (
                  <div className="flex flex-col items-end">
                    <span className={`text-xl font-bold ${isLeavingSoon ? 'text-brand' : 'text-foreground'}`}>
                      {Math.floor(dep.actualRelativeTime / 60)}
                    </span>
                    <span className="text-xs text-muted uppercase tracking-wider">Min</span>
                  </div>
                )}
              </div>
            </motion.div>
