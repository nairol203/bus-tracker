...
export default function Searchbar({ onSelectStop }: SearchbarProps) {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { setRecentStops(JSON.parse(saved)); } catch (e) {}
    }
  }, []);
  ...
  const handleSelect = (stop: Stop) => {
    ...
    // Update recent stops
    const newRecent = [stop, ...recentStops.filter(s => s.id !== stop.id)].slice(0, 5);
  ...
  const { data: stops = [], isLoading } = useSWR<Stop[]>('/api/stops', fetcher, {
    revalidateOnFocus: false, // Stops rarely change
    revalidateIfStale: false,
  });

  const fuse = useMemo(() => new Fuse(stops, {
    keys: ['name', 'number'],
    threshold: 0.3,
  }), [stops]);
  ...
  const suggestionsToDisplay = recentStops.length > 0 ? recentStops : recommendedStops;
  const suggestionLabel = recentStops.length > 0 ? 'Zuletzt gesucht' : 'Empfohlen';

  return (
    <div className="relative w-full max-w-2xl mx-auto z-50">
      <div className={`relative flex items-center w-full px-4 py-3 bg-surface border transition-colors duration-300 rounded-2xl shadow-lg ${isFocused ? 'border-brand' : 'border-border'}`}>
          ...
          >
            {showSuggestions && (
              <div className="px-4 py-2 bg-surface-hover/50 text-xs font-semibold text-muted uppercase tracking-wider flex items-center">
                {recentStops.length > 0 ? <Clock className="w-3 h-3 mr-2" /> : <Star className="w-3 h-3 mr-2" />}
                {suggestionLabel}
              </div>
            )}
            <ul className="max-h-64 overflow-y-auto py-2">
              {(showSuggestions ? suggestionsToDisplay : filteredStops).map((stop) => (
                <li key={stop.id} className="flex items-center hover:bg-surface-hover transition-colors px-2">
                  <button
                  ...
                  {showSuggestions && suggestionsToDisplay === recentStops && (
                    <button
                      onMouseDown={(e) => {
